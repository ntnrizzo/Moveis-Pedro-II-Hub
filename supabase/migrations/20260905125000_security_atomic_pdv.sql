BEGIN;
CREATE TABLE IF NOT EXISTS public.pdv_operations (
  id uuid PRIMARY KEY,
...
ALTER TABLE public.pdv_operations ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.pdv_operations FROM anon, authenticated;

-- Internal helper: preserve column defaults for fields omitted from JSON.
...
BEGIN
  IF p_table NOT IN ('public.vendas'::regclass,'public.solicitacoes_encomenda'::regclass) THEN RAISE EXCEPTION 'Tabela inválida'; END IF;
  IF EXISTS(SELECT 1 FROM jsonb_object_keys(p_data) k WHERE NOT EXISTS(
    ...
    RAISE EXCEPTION 'Campo não reconhecido no contrato da venda';
  END IF;
...
DECLARE org uuid; uid uuid:=auth.uid(); op_id uuid; op public.pdv_operations%ROWTYPE;
  ...
  variant_id uuid; changed integer; result jsonb; request_data jsonb; created_sale_id bigint;
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    SELECT to_jsonb(v) INTO STRICT result FROM public.vendas v WHERE id=op.venda_id AND organization_id=org;
    RETURN result;
  END IF;
  ...
  END IF;
  result := public._security_insert_json('public.vendas'::regclass,request_data);
  created_sale_id := (result->>'id')::bigint;
  UPDATE public.pdv_operations SET venda_id=created_sale_id, result=result WHERE id=op_id;
